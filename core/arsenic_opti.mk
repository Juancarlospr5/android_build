...
ARSENIC_GCC_CPPFLAGS := \
	-fvisibility-inlines-hidden

ARSENIC_CLANG_CFLAGS :=
ARSENIC_CLANG_CPPFLAGS :=
ARSENIC_CLANG_LDFLAGS :=
endif


# Extra flags
ifeq ($(ENABLE_EXTRAGCC),true)
  # ARSENIC_GCC_CFLAGS += -fgcse-las -fgraphite -ffast-math -fgraphite-identity -fgcse-sm -fivopts -fomit-frame-pointer -frename-registers -fsection-anchors -ftracer -ftree-loop-im -ftree-loop-ivcanon -funsafe-loop-optimizations -funswitch-loops -fweb -Wno-error=array-bounds -Wno-error=clobbered -Wno-error=maybe-uninitialized -Wno-error=strict-overflow -frerun-cse-after-loop -ffunction-sections -fdata-sections -fira-loop-pressure -fforce-addr -funroll-loops -ftree-loop-distribution -ffp-contract=fast -mvectorize-with-neon-quad -Wno-unused-parameter -Wno-unused-but-set-variable

   ARSENIC_GCC_CPPFLAGS += -fgcse-las -fgcse-sm -fivopts -fomit-frame-pointer -frename-registers -fsection-anchors -ftracer -ftree-loop-im -ftree-loop-ivcanon -funsafe-loop-optimizations -funswitch-loops -fweb -Wno-error=array-bounds -Wno-error=clobbered -Wno-error=maybe-uninitialized -Wno-error=strict-overflow -frerun-cse-after-loop -ffunction-sections -fdata-sections -fira-loop-pressure -fforce-addr -funroll-loops -ftree-loop-distribution -ffp-contract=fast -mvectorize-with-neon-quad -Wno-unused-parameter -Wno-unused-but-set-variable
endif

 #Graphite
ifeq ($(GRAPHITE_OPTS),true)
   ARSENIC_GCC_CPPFLAGS += -fgraphite -fgraphite-identity -floop-flatten -floop-parallelize-all -ftree-loop-linear -floop-interchange -floop-strip-mine -floop-block
...
ifeq ($(FFAST_MATH),true)
   ...
   # ARSENIC_GCC_CPPFLAGS += -ffast-math -ftree-vectorize
endif

# IPA Analyser
ifeq ($(ENABLE_IPA_ANALYSER),true)
       ARSENIC_GCC_CFLAGS += -fipa-sra -fipa-pta -fipa-cp -fipa-cp-clone
       ARSENIC_GCC_LDFLAGS += -fipa-sra -fipa-pta -fipa-cp -fipa-cp-clone
endif

# pipe
ifeq ($(TARGET_USE_PIPE),true)
   CLANG_CONFIG_EXTRA_CONLYFLAGS += -pipe
   ARSENIC_GCC_CPPFLAGS += -pipe
endif

# Krait
ifeq ($(KRAIT_TUNINGS),true)
    CLANG_CONFIG_EXTRA_CONLYFLAGS += -mcpu=cortex-a15 -mtune=cortex-a15
    ARSENIC_GCC_CPPFLAGS += -mcpu=cortex-a15 -mtune=cortex-a15
endif

# pthread
ifeq ($(ENABLE_PTHREAD),true)
   #ARSENIC_GCC_CFLAGS += -pthread
   ARSENIC_GCC_CPPFLAGSS += -pthread
endif

# OpenMP
ifeq ($(ENABLE_GOMP),true)
   ARSENIC_GCC_CFLAGS += -lgomp -ldl -lgcc -fopenmp
endif
...
ifeq ($(STRICT_ALIASING),true)
   ...
   ARSENIC_GCC_CPPFLAGS += -fstrict-aliasing -Werror=strict-aliasing -fno-strict-aliasing -Wstrict-aliasing=3
endif

# No error
ifeq ($(DONT_ERROROUT),true)
 ifneq ($(filter 4.8& 4.9% 4.9%,$(TARGET_GCC_VERSION)),)
    ARSENIC_GCC_CFLAGS += -Wno-error
    ARSENIC_GCC_CPPFLAGS += -Wno-error
 endif
endif
...
ARSENIC_IGNORE_RECOVERY_SIZE := true 
